...
path_detections = directorio_actual + '/RecortesDatasetCrestas2'
...
def predictandSegment(path_img):
    ...
    if bool(blag):  
        ...
        for r in results:
            ...
            box=box.xyxy
            coord=box.numpy()
            recorteContorno(path_img,path_to_txt2,out_path,nombre_sin_extension,coord)
    else:
        no_detected+=1
cantidadTotal=0
for carpeta in os.listdir('/home/dell/Documentos/YOLOv7Test/YOLOv8/dataset_crestas'):
    ...
    cantidadTotal=cantidadTotal+len(os.listdir(contenedora_ruta))
    for imagen in os.listdir(contenedora_ruta):
        imagen_ruta=os.path.join(contenedora_ruta,imagen)   
        predictandSegment(imagen_ruta)


print(f"Hay {cantidadTotal} imagenes en de las cuales.......")
porcentaje_validas=(detected*100)/cantidadTotal
porcentaje_novalidas=(no_detected*100)/cantidadTotal
